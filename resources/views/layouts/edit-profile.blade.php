<x-app>
        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="custom-container">
            ...
            <div class="auth-form profile-form">
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" cols="20" rows="4" class="form-control" name="description"
                        placeholder="Enter your description here" oninput="updateWordCount()">{{ auth()->user()->description }}</textarea>
                    <div class="text-end">
                        <small id="wordCount" class="form-text text-muted">0/50</small>
                    </div>
                </div>
    ...
    @push('scripts')
        ...
        <script>
            // Initialize word count on page load based on existing description value
            window.onload = function() {
                updateWordCount(); // Call the function to initialize word count
            };
            ...
            function updateWordCount() {
                ...
                document.getElementById('wordCount').textContent = `${wordCount}/${maxWords}`;

                // Optionally, limit the input to 50 words
                if (wordCount >= maxWords) {
                    document.getElementById('description').setAttribute('maxlength', descriptionText.length);
                } else {
                    document.getElementById('description').removeAttribute('maxlength');
                }
            }
        </script>
    @endpush
